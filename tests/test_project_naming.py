...
back.  This repository had no gate for it at all, even though it is the one that
names every cluster, node pool and package: nothing here would have noticed the
prefix reappearing in a Makefile, a catalog entry or a bootstrap script.

Two failure modes are guarded against explicitly, because both produce a green
test that protects nothing:

* a scan face that is too narrow.  Here it is every git-tracked path - the
  first-party-name gate in ``test_documentation.py`` lost Makefile, Dockerfile
  and bootstrap/git-server.Dockerfile to a suffix allowlist before it was
  widened the same way.  :class:`ScanFaceTests` asserts both that the odd
  corners are in the face and that files are really being read.
* a matcher that cannot fire.  :class:`MatcherTests` pins it in both
  directions, because the prefix is a substring of ``minio``, ``deterministic``
  and ``minimum`` - all legitimate, all present here, so a bare substring test
  is not an option and an over-broad one gets narrowed until it guards nothing.

The patterns fire on three shapes only: the prefix bound to a product name, the
retired environment-variable namespace, and the retired dotdir prefix.  Product
words are spelled in fragments for a second reason here: written out, this file
would be a hit of the first-party-name gate next door.
"""
