...
them abort, at the point of use rather than at the edit.

The arrays this bites hardest are the optional ones: an argument list that
stays empty unless you set an override is empty on exactly the default path a
newcomer takes. `warnings`/`problems` in scripts/preflight.sh are worse still,
because empty is the *success* case -- preflight crashed precisely when it had
nothing to report.
...
catches the shape every occurrence found in this repository had.
"""
...
def unguarded(text: str, name: str) -> list[int]:
    ...
    first version of this check reported every fixed line as still broken,
    which is the same answer it gives for a script nobody touched.
    """
    plain = f'"${{{name}[@]}}"'
    guard = f'${{{name}[@]+'
    hits = []
