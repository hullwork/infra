...
| Argo CD output | Application and ApplicationSet APIs; `helm.valuesObject` and per-set deletion controls | Alpha deployment baseline is Argo CD 3.5.2; earlier versions are not claimed supported |
...
| Kubernetes | A version supported by the chosen Argo CD release and workload | The compiler does not call Kubernetes; exact live test version is recorded in release readiness |
