...
released version is [`0.1.0-alpha.3`](https://github.com/hullwork/infra/releases/tag/v0.1.0-alpha.3), built from `7a2f8fc`. Only results actually executed are marked passed.
The copy inside a release archive is a prepublication snapshot; this document on
`main` records the subsequent download and provenance checks.

| Gate | Status | Evidence / limit |
| --- | --- | --- |
| Unit tests and fresh-clone install | Passed | 115 tests plus clean-clone install/validate/render passed in all four GitHub OS/Python jobs at `7a2f8fc` |
...
| GitHub Actions | Passed | At `7a2f8fc`, all steps completed: [core matrix](https://github.com/hullwork/infra/actions/runs/34248975599) and [CodeQL, dependency audit, history scan](https://github.com/hullwork/infra/actions/runs/34248975760). Earlier private-repository billing failures are superseded by these executed results |
...
| Release archive and anonymous download | Passed | All six `alpha.3` assets downloaded without credentials; every checksummed file matched; clean core extraction installed dependencies and validated/rendered both default and hello examples |
| Build provenance and SBOM | Passed | All six assets passed `gh attestation verify` constrained to this repository, release workflow, tag and exact source SHA, excluding self-hosted runners; all seven resolved Python dependency versions were present in the SPDX inventory; [release workflow](https://github.com/hullwork/infra/actions/runs/34249257158) passed |
...
Temporary acceptance namespaces, controllers, CRDs, Git server, port forwarding
and test container were removed after verification; logs were retained.
